  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await registerUser({
        ...
        credential_reference: formData.credential_reference,
        terms_accepted: true,
    ...
    <div className="app-shell flex items-center py-10">
      <div className="page-wrap">
        <Card className="glass-card mx-auto w-full max-w-xl border-slate-200">
          <CardHeader>
            <CardTitle className="text-2xl">Government Registration</CardTitle>
            <CardDescription>Create regulatory access credentials.</CardDescription>
          </CardHeader>
          <CardContent>
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div><Label>Credential Reference</Label><Input name="credential_reference" value={formData.credential_reference} onChange={handleInputChange} /></div>
              ...
              <Button className="w-full btn-primary" disabled={isLoading}>{isLoading ? "Creating..." : "Create Government Account"}</Button>
            </form>
            <p className="mt-4 text-center text-sm text-slate-600">Already have an account? <Link to="/government/login" className="text-primary">Sign in</Link></p>
          </CardContent>
        </Card>
      </div>
    </div>
