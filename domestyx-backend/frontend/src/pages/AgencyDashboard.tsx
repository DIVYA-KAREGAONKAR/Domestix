...
const AgencyDashboard = () => {
 ...
 const updateSubmissionStatus = async (submissionId: number, status: "submitted" | "verified" | "rejected", notes?: string) => {
 try {
 await api.patch(`/agency/worker-submissions/${submissionId}/`, {
 status,
 notes: notes || "",
 });
 ...
 toast({ title: "Error", description: "Unable to update submission status.", variant: "destructive" });
 }
 };
 ...
 <span className="text-sm text-gray-600">{user?.first_name || "Agency"}</span>
 ...
 <p className="font-medium text-app-text">Worker: {s.worker_name || s.worker}</p>
 <p className="text-sm text-gray-600">Role: {s.job_role || "N/A"}</p>
 <p className="text-sm text-gray-600">Experience: {s.experience_summary || "N/A"}</p>
 <p className="text-sm text-gray-600">Status: {s.status}</p>
 <p className="text-sm text-gray-600">Notes: {s.notes || "N/A"}</p>
 ...
 <Button size="sm" variant="outline" onClick={() => void updateSubmissionStatus(s.id, "submitted", s.notes)}>Submitted</Button>
 <Button size="sm" variant="outline" onClick={() => void updateSubmissionStatus(s.id, "verified", s.notes)}>Verified</Button>
 <Button size="sm" variant="outline" onClick={() => void updateSubmissionStatus(s.id, "rejected", s.notes)}>Rejected</Button>
 </div>
 </div>
 ))}
 </CardContent>
