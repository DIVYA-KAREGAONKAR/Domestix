...
const AgencyRegister = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!otpVerified || otpTarget !== currentOtpTarget) return setError(`Please verify your ${otpChannel} OTP before creating the account.`);
    ...
    try {
      await registerUser({
        ...
        mohre_approval_number: formData.mohre_approval_number,
        agency_contact_information: formData.agency_contact_information,
        terms_accepted: true,
      ...
      toast({ title: "Registration Successful", description: "Agency account created." });
  ...
  return (
    <div className="app-shell flex items-center py-10">
      <div className="page-wrap">
        <Card className="glass-card mx-auto w-full max-w-xl border-slate-200">
          <CardHeader>
            <CardTitle className="text-2xl">Agency Registration</CardTitle>
            <CardDescription>Create agency credentials and profile.</CardDescription>
          </CardHeader>
          <CardContent>
            <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              <div><Label>Contact Information</Label><Input name="agency_contact_information" value={formData.agency_contact_information} onChange={handleInputChange} /></div>
              ...
              <Button className="w-full btn-primary" disabled={isLoading}>{isLoading ? "Creating..." : "Create Agency Account"}</Button>
            </form>
            <p className="mt-4 text-center text-sm text-slate-600">Already have an account? <Link to="/agency/login" className="text-primary">Sign in</Link></p>
          </CardContent>
        </Card>
      </div>
    </div>
