      ...
      <header className="border-b border-border/70 bg-white/90 backdrop-blur">
        <div className="page-wrap flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-sm text-slate-600">{user?.first_name || "Government"}</span>
      ...
      <main className="page-wrap space-y-6 py-6">
        <Card className="glass-card border-slate-200">
          ...
          <CardContent className="grid grid-cols-2 gap-3 md:grid-cols-4">
            <div className="rounded-lg border p-3"><p className="text-xs text-slate-500">Jobs</p><p className="text-xl font-bold">{analytics?.totals?.jobs || 0}</p></div>
            <div className="rounded-lg border p-3"><p className="text-xs text-slate-500">Applications</p><p className="text-xl font-bold">{analytics?.totals?.applications || 0}</p></div>
            <div className="rounded-lg border p-3"><p className="text-xs text-slate-500">Hired</p><p className="text-xl font-bold">{analytics?.totals?.hired || 0}</p></div>
            <div className="rounded-lg border p-3"><p className="text-xs text-slate-500">Open Reports</p><p className="text-xl font-bold">{analytics?.totals?.compliance_reports_open || 0}</p></div>
          </CardContent>
        </Card>
        ...
        <Card className="glass-card border-slate-200">
          ...
          <CardContent className="space-y-3">
            {jobReviews.length === 0 ? (
              <p className="text-sm text-slate-500">No pending jobs for review.</p>
            ) : jobReviews.map((j) => (
              <div key={j.id} className="rounded-lg border p-3 space-y-2">
                <p className="font-semibold text-slate-900">{j.title}</p>
                <p className="text-sm text-slate-600">Location: {j.location}</p>
                <p className="text-sm text-slate-600">Salary: {j.salary}</p>
                <p className="text-sm text-slate-600">Status: {j.review_status}</p>
        ...
        <Card className="glass-card border-slate-200">
          ...
          <CardContent className="space-y-3">
            {reports.length === 0 ? (
              <p className="text-sm text-slate-500">No compliance reports found.</p>
            ) : reports.map((r) => (
              <div key={r.id} className="rounded-lg border p-3 space-y-2">
                <p className="font-semibold text-slate-900">Report #{r.id} · {r.category}</p>
