...
const EmployerDashboard = () => {
 ...
 <header className="sticky top-0 z-20 border-b border bg-white ">
 ...
 <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 py-6 md:py-8">
 <div className="flex flex-col justify-between gap-3 md:flex-row md:items-center">
 <h1 className="text-2xl font-semibold text-app-text">Welcome, {user.first_name || "Employer"}!</h1>
 <div className="flex w-full flex-wrap gap-1 rounded-xl border border bg-secondary/60 p-1 md:w-auto">
 ...
 <Card><CardContent className="p-4"><p className="text-xs text-gray-500">Jobs Posted</p><p className="text-xl font-bold">{metrics.totalJobs}</p></CardContent></Card>
 <Card><CardContent className="p-4"><p className="text-xs text-gray-500">Applicants</p><p className="text-xl font-bold">{metrics.totalApplicants}</p></CardContent></Card>
 <Card><CardContent className="p-4"><p className="text-xs text-gray-500">Accepted</p><p className="text-xl font-bold text-green-600">{metrics.hiredCount}</p></CardContent></Card>
 <Card><CardContent className="p-4"><p className="text-xs text-gray-500">Rejected</p><p className="text-xl font-bold text-red-600">{metrics.rejectedCount}</p></CardContent></Card>
 </div>
 ...
 <h2 className="font-semibold text-lg text-gray-700">Your Job Postings</h2>
 {jobs.length === 0 ? (
 <div className="p-12 border-2 border-dashed rounded-xl text-center text-gray-400">No jobs posted yet.</div>
 ) : jobs.map((job: any) => (
 ...
 <h2 className="font-semibold text-lg text-gray-700 mb-4">Applicant Review</h2>
 {!selectedJob ? (
 <div className="p-8 border-2 border-dashed rounded-xl text-center text-gray-400">Select a job to see applicants.</div>
 ) : (
 ...
 <p className="text-xs text-gray-600 mt-1">Publish Review: {selectedJob.review_status || "pending"}</p>
 ...
 <div className="rounded-xl border border-border/60 bg-secondary/40 p-8 text-center text-gray-500">No applicants yet.</div>
 ...
 <p className="font-bold text-sm text-app-text truncate">{app.worker_details?.name}</p>
 <p className="text-xs text-gray-500">{app.worker_details?.experience || "No experience listed"}</p>
 </div>
 </div>
 ...
 <h2 className="font-semibold text-lg text-gray-700">Recommended Workers (AI Match)</h2>
 {!selectedJob ? (
 <Card><CardContent className="p-6 text-gray-400">Select a job to load recommendations.</CardContent></Card>
 ) : recommendedWorkers.length === 0 ? (
 <Card><CardContent className="p-6 text-gray-400">No recommended workers found for this job yet.</CardContent></Card>
 ) : (
 ...
 <div>
 <p className="font-semibold text-app-text">{worker.name}</p>
 <p className="text-xs text-gray-500">Experience: {worker.experience || "N/A"}</p>
 </div>
 ...
 {viewMode === "history" && (
 <div className="space-y-4">
 <h2 className="font-semibold text-lg text-gray-700">Decision History</h2>
 {history.length === 0 ? (
 <Card className="p-12 text-center text-gray-400">No past decisions recorded.</Card>
 ...
 <div className="min-w-0">
 <p className="font-bold text-app-text truncate">{item.worker_details?.name}</p>
 <p className="text-sm text-gray-500 truncate">Job: {item.job_details?.title}</p>
 </div>
 </div>
 ...
 {viewMode === "offers" && (
 <div className="space-y-4">
 <h2 className="font-semibold text-lg text-gray-700">Sent Offers</h2>
 {offers.length === 0 ? (
 <Card><CardContent className="p-8 text-gray-400 text-center">No offers sent yet.</CardContent></Card>
 ) : offers.map((offer: any) => (
 <Card key={offer.id}>
 <CardContent className="p-4 space-y-2">
 <p className="font-semibold text-app-text">{offer.job_title}</p>
 <p className="text-sm text-gray-600">Worker: {offer.worker_name || offer.worker}</p>
 <p className="text-sm text-gray-600">Status: {offer.status}</p>
 <p className="text-sm text-gray-600">Message: {offer.message || "N/A"}</p>
 <p className="text-sm text-gray-600">Employer Signature: {offer.employer_signature_name || "Not signed"}</p>
 <p className="text-sm text-gray-600">Worker Signature: {offer.worker_signature_name || "Not signed"}</p>
 <div className="flex gap-2">
 ...
 {viewMode === "shortlist" && (
 <div className="space-y-4">
 <h2 className="font-semibold text-lg text-gray-700">Saved / Shortlisted Workers</h2>
 {shortlistedWorkers.length === 0 ? (
 <Card><CardContent className="p-10 text-center text-gray-400">No shortlisted workers yet.</CardContent></Card>
 ...
 <p className="text-xs text-gray-500 truncate">{item.job_details?.title || "General shortlist"}</p>
 ...
 <div className="space-y-4">
 <div className="flex items-center justify-between">
 <h2 className="font-semibold text-lg text-gray-700">Compare Workers</h2>
 <Button onClick={() => void handleRunCompare()}><Check className="w-4 h-4 mr-1" /> Run Compare</Button>
 </div>
 <p className="text-sm text-gray-500">Select up to 3 shortlisted workers and compare profile, skills, and hiring history.</p>

 {shortlistedWorkers.length === 0 ? (
 <Card><CardContent className="p-8 text-gray-400 text-center">Shortlist workers first to compare.</CardContent></Card>
 ) : (
 ...
 <p className="text-xs text-gray-500">{item.worker_details?.experience || "N/A"}</p>
 </div>
 ...
 <p className="text-sm text-gray-500">Experience: {worker.experience || "N/A"}</p>
 <p className="text-sm text-gray-500">Gender: {worker.gender || "N/A"}</p>
 <p className="text-sm text-gray-500">Nationality: {worker.nationality || "N/A"}</p>
 <p className="text-sm text-gray-500">Job Roles: {(worker.job_roles || worker.services || []).join(", ") || "N/A"}</p>
 <p className="text-sm text-gray-500">Skills: {(worker.services || []).join(", ") || "N/A"}</p>
 <p className="text-sm text-gray-500">Languages: {(worker.languages || []).join(", ") || "N/A"}</p>
 <p className="text-sm text-gray-500">Availability: {(worker.availability || []).join(", ") || "N/A"}</p>
 <p className="text-sm text-gray-500">Work Preference: {worker.work_preference || "N/A"}</p>
 <p className="text-sm text-gray-500">Preferred Locations: {(worker.preferred_work_locations || []).join(", ") || "N/A"}</p>
 <p className="text-sm text-gray-500">Willing to Relocate: {worker.willing_to_relocate ? "Yes" : "No"}</p>
 <p className="text-sm text-gray-500">Expected Salary (hourly): {worker.expected_salary_hourly ? `AED ${worker.expected_salary_hourly}` : "N/A"}</p>
 <p className="text-sm text-gray-500">Expected Salary (full-time): {worker.expected_salary_full_time ? `AED ${worker.expected_salary_full_time}` : "N/A"}</p>
 <p className="text-sm text-gray-500">Expected Salary (part-time): {worker.expected_salary_part_time ? `AED ${worker.expected_salary_part_time}` : "N/A"}</p>
 <p className="text-sm text-gray-500">Expected Benefits: {(worker.expected_benefits || []).join(", ") || "N/A"}</p>
 <p className="text-sm text-gray-500">Available From: {worker.available_from || "N/A"}</p>
 <p className="text-sm text-gray-500">Location: {[worker.location?.city, worker.location?.state, worker.location?.country].filter(Boolean).join(", ") || "N/A"}</p>
 <p className="text-sm text-gray-500">Verified: {worker.is_verified ? "Yes" : "No"}</p>
 <p className="text-sm text-gray-500">Visa Type: {worker.visa_type || "N/A"}</p>
 <p className="text-sm text-gray-500">Work Permit: {worker.work_permit_status || "N/A"}</p>
 <p className="text-sm text-gray-500">Police Clearance: {worker.police_clearance_available ? "Yes" : "No"}</p>
 <p className="text-sm text-gray-500">Criminal Record: {worker.has_criminal_record ? "Yes" : "No"}</p>
 <p className="text-sm text-gray-500">Applications: {worker.total_applications || 0}</p>
 <p className="text-sm text-gray-500">Hired: {worker.total_hired || 0}</p>
 ...
 <CardContent className="p-4 space-y-3">
 <h2 className="font-semibold text-lg text-gray-700">Conversations</h2>
 {threads.length === 0 ? (
 <p className="text-sm text-gray-500">No conversations yet. Start chat from History or Shortlist.</p>
 ) : (
 ...
 >
 <p className="font-medium text-app-text truncate">{thread.worker_name || "Worker"}{thread.job_title ? ` · ${thread.job_title}` : ""}</p>
 <p className="text-xs text-gray-500 truncate">{thread.last_message?.message || "No messages yet"}</p>
 ...
 <CardContent className="p-4 space-y-3">
 <h2 className="font-semibold text-lg text-gray-700">Messages</h2>
 {!activeThreadId ? (
 ...
 <div key={msg.id} className={`max-w-[80%] rounded-lg px-3 py-2 text-sm ${msg.sender === user?.id ? "ml-auto bg-primary text-primary-foreground" : "bg-secondary text-gray-800"}`}>
 <p>{msg.message}</p>
 <p className={`mt-1 text-[10px] ${msg.sender === user?.id ? "text-primary-foreground/80" : "text-gray-500"}`}>
 ...
 <h2 className="font-semibold text-lg text-gray-700">Contract Generator</h2>
 ...
 <h2 className="font-semibold text-lg text-gray-700 mb-3">Contract Preview</h2>
 <pre className="min-h-[420px] overflow-auto whitespace-pre-wrap rounded-lg border border-border/60 bg-secondary/40 p-4 text-sm text-gray-700">
