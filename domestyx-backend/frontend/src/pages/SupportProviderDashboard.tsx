...
const SupportProviderDashboard = () => {
 ...
 <span className="text-sm text-gray-600">{user?.first_name || "Support Provider"}</span>
 <Button variant="outline" size="sm" onClick={() => { logout(); navigate("/"); }}>Logout</Button>
 </div>
 </div>
 </header>

 <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 py-6">
 <Card>
 ...
 <p className="text-sm font-semibold text-app-text">{categoryText || "Not set"}</p>
 </div>
 <div className="rounded-lg border border-border/60 bg-secondary/30 p-3">
 ...
 {requests.length === 0 ? (
 <p className="text-sm text-gray-500">No service requests assigned yet.</p>
 ) : requests.map((r) => (
 <div key={r.id} className="rounded-lg border p-3 space-y-2">
 <p className="font-medium text-app-text">Request #{r.id} · {r.service_type}</p>
 <p className="text-sm text-gray-600">Requester: {r.requester_name || r.requester}</p>
 <p className="text-sm text-gray-600">Details: {r.details || "N/A"}</p>
 ...
 <CardContent className="space-y-3">
 {!activeRequestId ? (
 <p className="text-sm text-gray-500">Select a request and click Open Chat.</p>
 ) : (
 <>
 ...
 <div key={m.id} className={`max-w-[85%] rounded-lg px-3 py-2 text-sm ${m.sender === user?.id ? "ml-auto bg-primary text-primary-foreground" : "bg-secondary text-gray-800"}`}>
 <p>{m.message}</p>
 <p className={`mt-1 text-[10px] ${m.sender === user?.id ? "text-primary-foreground/80" : "text-gray-500"}`}>
